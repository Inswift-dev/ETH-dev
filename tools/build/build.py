...
def after_build(source, target, env):
    ...
    for f in glob("bin/inswift_*.bin"):
        os.unlink(f)

    exit_code = call(
        "python tools/build/merge_bin_esp.py --output_folder ./bin --output_name inswift.full.bin --bin_path bin/bootloader_dio_40m.bin bin/partitions.bin bin/firmware.bin bin/inswift.fs.bin --bin_address 0x1000 0x8000 0x10000 0x290000",
        shell=True,
    )
    ...
    NEW_NAME_BASE = "bin/inswift_" + VERSION_NUMBER
    ...
    shutil.move("bin/inswift.full.bin", NEW_NAME_FULL)
    shutil.move("bin/firmware.bin", NEW_NAME_OTA)
    shutil.move("bin/inswift.fs.bin", NEW_NAME_FS)
