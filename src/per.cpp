...
void handleLongBtn()
{
    if (digitalRead(hwConfig.mist.btnPin) != hwConfig.mist.btnPlr)
    {
        ...
        //     ledControl.modeLED.mode = LED_BLINK_5T;

        // }
        if (btnFlag >= 10)
        {   
            LOGD("press %d s, factoryReset", btnFlag);
            factoryReset();
        }
    }
    else
...
void ledStatusSetup()
{
    ...
    ledControl.statusLED.name = "status";
    ledControl.statusLED.pin = 4;
    ledControl.statusLED.active = true;
    ledControl.statusLED.mode = LED_OFF;

    LOGD("%d", ledControl.statusLED.mode);

    xTaskCreate(ledTask, "STATUS LED Task", 2048, &ledControl.statusLED, 10, NULL);
}