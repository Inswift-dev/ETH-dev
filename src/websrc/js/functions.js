...
const zbFwInfoUrl = "https://raw.githubusercontent.com/Inswift-dev/inswift-eth/main/ti-cc2652/manifest.json";
...
const pages = {
	...
	API_PAGE_MQTT: { num: 4, str: "/mqtt" },
	API_PAGE_VPN: { num: 5, str: "/vpn" },
	API_PAGE_SECURITY: { num: 6, str: "/security" },
	API_PAGE_TOOLS: { num: 7, str: "/tools" },
...
function localizeTitle(url) {
	...
	$("title[data-r2v='pageName']").text(page_title + " - Inswift");//update page title
}
				...
				click: function () {
					...
					$("<div>", {
						...
						append: $("<div>", {
							"class": "progress-bar progress-bar-striped progress-bar-animated",
							id: "prg",
							style: "width: 0%; background-color: #f7de01ff;"
						})
					}).appendTo(modalBody);
					if (action == 1) {
						$.ajax({
		...
		case "espGitVersions":
			...
			params.releases.forEach(release => {
				...
				if (release.assets.length > 0) {
					const downloadLink = release.assets[2].browser_download_url;
					$('<a>', {
...
async function fetchReleaseData() {
	...
	var t = await fetch("https://api.github.com/repos/Inswift-dev/inswift-eth/releases");
