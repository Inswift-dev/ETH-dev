...
#define FW_UPDATE_URL_BASE NULL //"https://github.com/xyzroe/XZG/releases/download/"
#define FW_UPDATE_URL_RELEASE_NAME NULL  //"XZG_"
#define FW_UPDATE_RELEASE_MANIFEST_URL NULL   //"https://raw.githubusercontent.com/xyzroe/XZG/refs/heads/releases/latest/xzg.json"
#define ZB_UPDATE_RELEASE_MANIFEST_URL NULL // "https://raw.githubusercontent.com/Inswift-dev/inswift-eth/main/ti-cc2652/manifest.json"
#endif
...
bool captivePortal()
{
    if ((vars.apStarted) && (!vars.connectedEther))
    {
        IPAddress ip;
...
static void apiCmdZbCheckFirmware(String &result)
{
    if (zbFwCheck())
    {
        printLogMsg("[RCP] " + String(CCTool.chip.fwRev));
        serverWeb.send(HTTP_CODE_OK, contTypeText, result);
    }
    else
    {
...
void runEspUpdateFirmware(uint8_t *data, size_t len)
{
    ...
    if (Update.end()) {
        ...
        Serial.print("ota erro：0x");
        Serial.println(Update.getError(), HEX);
      }
