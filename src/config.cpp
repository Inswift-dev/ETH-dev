...
void loadHwConfig(ThisConfigStruct &config)
{
    ...
    strlcpy(config.board, preferences.getString(boardKey, "Inswift").c_str(), sizeof(config.board));

    config.eth.addr = preferences.getInt(addrKey, 1);
    config.eth.pwrPin = preferences.getInt(pwrPinKey, 5);
    config.eth.mdcPin = preferences.getInt(mdcPinKey, 23);
    config.eth.mdiPin = preferences.getInt(mdiPinKey, 18);
    config.eth.phyType = static_cast<eth_phy_type_t>(preferences.getInt(phyTypeKey, ETH_PHY_TLK110));
    ...
    config.zb.txPin = preferences.getInt(zbTxPinKey, 17);
    config.zb.rxPin = preferences.getInt(zbRxPinKey, 13);
    config.zb.rstPin = preferences.getInt(zbRstPinKey, 15);
    config.zb.bslPin = preferences.getInt(zbBslPinKey, 33);

    config.mist.btnPin = preferences.getInt(btnPinKey, 34);
    ...
    config.mist.ledModePin = preferences.getInt(ledModePinKey, 2);
    config.mist.ledModePlr = preferences.getInt(ledModePlrKey, 0);
    config.mist.ledPwrPin = preferences.getInt(ledPwrPinKey, 14);
    config.mist.ledPwrPlr = preferences.getInt(ledPwrPlrKey, 0);
...
void saveSystemConfig(const SystemConfigStruct &config)
{
    ...
    if(String(config.hostname) == NULL)
    {
        // LOGD(" putString dongle %d",preferences.putString(hostnameKey, "dongle"));
        preferences.putString(hostnameKey, "dongle");
    }
    else
    {
        // LOGD(" wtha's happen!!!!!!!!!");
    ...
    preferences.putBool(zbSaveinfoKey, config.zbSaveinfo);
    preferences.putString(zbFwKey, config.zbFw);
    preferences.putString(zbHwKey, config.zbHw);
    preferences.putString(zbIeeeKey, config.zbIeee);
    preferences.putUInt(zbFlashSizeKey, config.zbFlashSize);
...
void loadSystemConfig(SystemConfigStruct &config)
{
    ...
    config.zbSaveinfo = preferences.getBool(zbSaveinfoKey, false);
    strlcpy(config.zbFw, preferences.getString(zbFwKey, "?").c_str(), sizeof(config.zbFw));
    strlcpy(config.zbHw, preferences.getString(zbHwKey, "?").c_str(), sizeof(config.zbHw));
    strlcpy(config.zbIeee, preferences.getString(zbIeeeKey, "?").c_str(), sizeof(config.zbIeee));
    config.zbFlashSize = preferences.getUInt(zbFlashSizeKey, 0);
...
{
    preferences.begin(systemConfigKey, true);
    strlcpy(sysConfig.hostname, preferences.getString(hostnameKey, "dongle").c_str(), sizeof(sysConfig.hostname));
    preferences.end();
    preferences.begin(vpnConfigKey, true);
    ...
    preferences.end();
    LOGD(" hostname %s, hnHostName %s, topic %s", sysConfig.hostname,vpnConfig.hnHostName,mqttConfig.topic);
    // strlcpy(sysConfig.hostname, vars.deviceId, sizeof(sysConfig.hostname));
    // strlcpy(vpnConfig.hnHostName, vars.deviceId, sizeof(vpnConfig.hnHostName));
    // strlcpy(mqttConfig.topic, vars.deviceId, sizeof(mqttConfig.topic));
}
...
void serializeSystemConfigToJson(const SystemConfigStruct &config, JsonObject obj)
{
    ...
    obj[zbSaveinfoKey] = config.zbSaveinfo;
    obj[zbFwKey] = config.zbFw;
    obj[zbHwKey] = config.zbHw;
    obj[zbIeeeKey] = config.zbIeee;
    obj[zbFlashSizeKey] = config.zbFlashSize;
