...
{
    char id_str[MAX_DEV_ID_LONG] = "dongle-";
...
ThisConfigStruct *findBrdConfig(int searchId = 0)
{
  ...
  bestConfig.eth = {.addr = -1, .pwrPin = -1, .mdcPin = -1, .mdiPin = -1, .phyType = ETH_PHY_TLK110, .clkMode = ETH_CLOCK_GPIO0_IN}; // .pwrAltPin = -1};
  ...
  int ethIdx = brdConfigs[searchId].ethConfigIndex;
  int zbIdx = brdConfigs[searchId].zbConfigIndex;
  int mistIdx = brdConfigs[searchId].mistConfigIndex;

  LOGI(" find brd: %d - %s", searchId, brdConfigs[searchId].board);
...
//   }
  ...
  strlcpy(bestConfig.board, brdConfigs[searchId].board, sizeof(bestConfig.board));
  return &bestConfig;
  ...
  // LOGI("Config error with: %s", brdConfigs[searchId].board);
  // restartDevice();
}
...
void ledTask(void *parameter)
{
  ...
  while (1)
  {
    // LOGI("%d | led %s | m %d", millis(), led->name, led->mode);
    if (led->pin == -1)
      continue;
    if (!led->active)
    {
